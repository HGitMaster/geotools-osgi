...
final class MlibXorOpImage extends PointOpImage {
    ...
    /**
     * Constructs an MlibXorOpImage. The image dimensions are copied
     ...
     */
    public MlibXorOpImage(RenderedImage source1, RenderedImage source2,
                          Map config,
			  ImageLayout  layout) {
    ...
    /**
     * Xor the pixel values of rectangles from the two sources.
	...
	MediaLibAccessor srcAccessor2 = 
	    ...
	    mediaLibImage[] srcML2 = srcAccessor2.getMediaLibImages();
            ...
            for (int i = 0 ; i < dstML.length; i++) {
                Image.Xor(dstML[i], srcML1[i], srcML2[i]);
            }
            break;
        default:
...
//         System.out.println("MlibXorOpImage Test");
//         ImageLayout layout;
...
//         dst = new MlibXorOpImage(src1, src2, null, null);
