/*
 * $RCSfile: MlibSubtractOpImage.java,v $
 *
...
/**
 * An OpImage that performs the Subtract operation on 2 images through mediaLib.
 *
 */
final class MlibSubtractOpImage extends PointOpImage {

    /**
     * Constructs an MlibSubtractOpImage. The image dimensions are copied
     ...
     */
    public MlibSubtractOpImage(RenderedImage source1, RenderedImage source2,
                               Map config,
                               ImageLayout layout) {
    ...
    /**
     * Subtract the pixel values of a rectangle from the two sources.
	...
	MediaLibAccessor srcAccessor2 = 
        ...
        case DataBuffer.TYPE_INT:
            for (int i = 0 ; i < dstML.length; i++) {
                Image.Sub(dstML[i], srcML1[i], srcML2[i]);
            }
            break;
        ...
        case DataBuffer.TYPE_DOUBLE:
            for (int i = 0 ; i < dstML.length; i++) {
                Image.Sub_Fp(dstML[i], srcML1[i], srcML2[i]);
            }
            break;
...
//         System.out.println("MlibSubtractOpImage Test");
//         ImageLayout layout;
...
//         dst = new MlibSubtractOpImage(src1, src2, null, null);
