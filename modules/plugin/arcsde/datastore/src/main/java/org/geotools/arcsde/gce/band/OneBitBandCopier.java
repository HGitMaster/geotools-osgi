...
public class OneBitBandCopier extends ArcSDERasterBandCopier {
     ...
     *      in there to be ignored by the WritableRaster implementation.
     * 
    ...
    public void copyPixelData(SeRasterTile tile, WritableRaster raster, int copyOffX, int copyOffY,
        ...
        final int bytesPerTileRow = (raster.getWidth() - imageDataBufOffset + 7) / 8; // the total
                                                                                      ...
                                                                                      // sub-image
                                                                                      // (tile)
        final int bytesPerDataRow = (tileWidth + 7) / 8; // the total number of bytes that make up
                                                         // one row in this data tile
    ...
    private byte[] copyArrayWithBitShift(byte[] src, int sstartindex, int sstartoffset,
            byte[] dest, int dstartindex, int dstartoffset, int length, boolean clipSource) {
