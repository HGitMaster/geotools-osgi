...
class UnsignedByteBandCopier extends ArcSDERasterBandCopier {
	...
	public void copyPixelData(SeRasterTile tile, WritableRaster raster, int copyOffX, int copyOffY, int targetBand) throws DataSourceException {
		if (LOGGER.isLoggable(Level.FINER))
			LOGGER.finer("copying raster data band " + tile.getBandId().longValue() + " into image band " + targetBand);
		...
		final int imgWidth = raster.getWidth() + copyOffX > tileWidth ? tileWidth - copyOffX : raster.getWidth();
		final int imgHeight = raster.getHeight() + copyOffY > tileHeight ? tileHeight - copyOffY : raster.getHeight();
			...
			for (y = 0; y < imgHeight; y++) {
				...
				try {
					...
					if (transferType == DataBuffer.TYPE_BYTE || transferType == DataBuffer.TYPE_INT) {
						...
						/*
						 * if (bandSample != 0 && Math.random() > .99995) { LOGGER.info("pixel " + x + "," + y + ", band:" + targetBand + " has value " + bandSample); }
						 */
					...
					LOGGER.severe("at data " + (x + copyOffX) + "," + (y + copyOffY) + "(img pixel " + x + "," + y + ")");
