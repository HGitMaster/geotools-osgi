...
class ShortBandCopier extends ArcSDERasterBandCopier {
    ...
    public void copyPixelData(SeRasterTile tile, WritableRaster raster, int copyOffX, int copyOffY,
        ...
        if (raster.getTransferType() != DataBuffer.TYPE_SHORT) {
            // this can't happen, it'd have been caught earlier when we created the transfer object.
            throw new IllegalArgumentException(
                    "Can't copy ArcSDE Raster data from an SE_PIXEL_TYPE_16BIT_S raster to a "
                            + "java.awt.Raster with a transferType of " + raster.getTransferType());
        }
        ...
        for (int x = 0; x < imgWidth; x++) {
            for (int y = 0; y < imgHeight; y++) {
                ...
                bandSample = (short) (pixelData[pixArrayOffset] & 0xFFFF);
