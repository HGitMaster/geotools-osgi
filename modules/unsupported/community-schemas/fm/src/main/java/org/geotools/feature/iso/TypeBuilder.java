 ...
 * <p>
 * For reference the builder captures the following state: <table border=1
 ...
 * <li>{@link init( PropertyDescriptor )} - completly load settings from
 * descriptor
 * <li>{@link init()} - completly replace settings with builder defaults
 * <ul>
 * <li>{@link reset()} - called after a type creation methods to reset common
 * type settings
 * <li>{@link clear()} - called after an descriptor create method (or add
 * methods) to clear common descriptor settings
 * </ul>
 * </ul>
 * <p>
 * For examples of using this class please see the following type creation
 * methods:
 * <ul>
 * <li>{@link attribute()}
 * <li>{@link geometry()}
...
public class TypeBuilder {
	 ...
	 */
	private Collection/* <StructuralDescriptor> */properties;
	 ...
	 * </ul>
	 * <p>
	 * Note: type type of collection class is not reset, a new instanceof the
	 * existing collection class is used.
	 */
	public void reset() {
		this.name = null;
	...
	public void init(PropertyDescriptor descriptor) {
		...
		if (descriptor instanceof StructuralDescriptor) {
			...
			minOccurs = structure.getMinOccurs();
			maxOccurs = structure.getMaxOccurs();
		}
	...
	public void init(PropertyType type) {
		...
		if (type instanceof AssociationType) {
			...
			referenceType = assType.getReferenceType();
		...
		if (type instanceof ComplexType) {
			...
			properties = null;
			properties.addAll(cType.attributes());
		}
		if (type instanceof FeatureType) {
			FeatureType featureType = (FeatureType) type;
			defaultGeom = featureType.getDefaultGeometry().getName();
	...
	public AttributeType attribute() {
		AttributeType type = factory.createAttributeType(typeName(),
				getBinding(), isIdentified(), isAbstract(), restrictions(),
	...
	public AssociationType association() {
		return factory.createAssociationType(typeName(), getReferenceType(),
				true, true, this.restrictions, getAssociationSuper(), this
						.getDescription());
	}
	 ...
	 * </code></pre>
	 * 
	 * Use of GeoAPI Geometry interfaces is encouraged as implementations are
	 * made avaiable.
	 * 
	 */
	public GeometryType geometry() {
		return getTypeFactory().createGeometryType(typeName(), getBinding(),
				getCRS(), isIdentified(), isAbstract(), restrictions(),
				getSuper(), getDescription());
	}
	 ...
	 * 
	 * <pre><code>
	 * ComplexType FULLNAME = builder.name(&quot;FullName&quot;).attribute(&quot;first&quot;, TEXT)
	 * 		.cardinality(0, Integer.MAX_VALUE).attribute(&quot;middle&quot;, TEXT).attribute(
	 * 				&quot;last&quot;, TEXT).complex();
	 * </code></pre>
	 * 
	 * @return ComplexType
	...
	public ComplexType complex() {
		ComplexType type = getTypeFactory().createComplexType(typeName(),
				properties(), isIdentified(), isAbstract(), restrictions(),
				getSuper(), getDescription());
		reset();
		return type;
	}
	...
	/**
	 * Create an AssociationDesctiptor, define relationships between
	 * ComplexAttribtues (in particular FeatureCollection to members).
	 * <p>
	 * Example:
	 * 
	 * <pre><code>
	 * AttributeDescriptor contains = build.name(&quot;contains&quot;).property(ROAD).nillable(
	 * 		false).cardinality(0, Interger.MAX_VALUE).associationDescriptor();
	 * </code></pre>
	 * 
	 * @return AttributeDescriptor used to define sturcture of ComplexAttribtues
	 */
	public AssociationDescriptor associationDescriptor() {
		AssociationDescriptor association = getTypeFactory()
				.createAssociationDescriptor((AssociationType) propertyType,
						typeName(), getMinOccurs(), getMaxOccurs());
		reset();
		return association;
	}
	...
	public FeatureType feature() {
		...
		AttributeDescriptor defaultGeometry = defaultGeometry();
		CoordinateReferenceSystem crs = getCRS();
		boolean isAbstract = isAbstract();
		InternationalString description = getDescription();
		AttributeType superType = getSuper();
		Set restrictions = restrictions();
		FeatureType type = typeFactory.createFeatureType(typeName, properties,
				defaultGeometry, crs, isAbstract, restrictions, superType,
				description);
		reset();
		return type;
	}
	...
	public FeatureCollectionType collection() {
		FeatureCollectionType type = getTypeFactory()
				.createFeatureCollectionType(typeName(), properties(),
						members(), defaultGeometry(), getCRS(), isAbstract(),
						restrictions(), getSuper(), getDescription());
		reset();
		return type;
	}
	...
	/**
	 * Accessor for restructions which does the null check and creation if
	 * necessary.
	 */
	...
	/**
	 * Template method to enable subclasses to customize the set implementation
	...
	public TypeBuilder attribute(String name, String namespaceURI,
			AttributeType type) {
	...
	public void addAttribute(String name, String namespaceURI,
			AttributeType type) {
	...
	/**
	 * Allows you to indicate the reference type to be used with Association to
	 * be created.
	 */
	...
	public TypeBuilder association(String namespaceURI, String name,
			AssociationType type) {
		return association(createName(namespaceURI, name), type);
	}

	public TypeBuilder association(Name name, AssociationType type) {
		AssociationDescriptor descriptor = getTypeFactory()
				.createAssociationDescriptor(type, name, getMinOccurs(),
						getMaxOccurs());
	 ...
	 * <p>
	 * Please note that you may not have two types with the same name, depending
	 * on the factory being used the order of the structural content may be
	 * signficant - this builder will preserve order although the factory may or
	 * may not make use of this fact.
	 * </p>
	 */
	 ...
	 * <li>etc...
	 * </ul>
	 * The collection class used here should be matched by content described by
	 * this type. Explicitly a FeatureType with
	 * <code>getProperties() instanceof Set</code> indicates that Features of
	 * that FeatureType should maintain a Set of properties where order is not
	 * significant.
	 * </p>
	 * 
	 * @param properties
	...
	//
	/**
	 * Template method to enable subclasses to customize the collection
	 * implementation used by "default".
	 * <p>
	 * Considered moving this to the type interface but it would be in
	 * appropriate as the user may need to specifiy different collections for
	 * seperate types in the same schema.
	 * </p>
	 * 
	 ...
	 * use
	 * </ul>
	 * This allows a user to indicate that properties are stored in a
	 * "LinkedList" once.
	 * 
	 * @param origional
	...
	/**
	 * Convenience method for getting the descriptor of the default geometry
	 * type. This method will first try to look up the supplied
	 * <code>defaultGeom</code> property, if it cant find, it will try to
	 * locate any added geometry.
	 * 
	 */
	protected AttributeDescriptor defaultGeometry() {
		if (defaultGeom != null) {
			for (Iterator itr = getProperties().iterator(); itr.hasNext();) {
				AttributeDescriptor ad = (AttributeDescriptor) itr.next();
				if (ad.getName().equals(defaultGeom)) {
					return ad;
				}
			}
		}
	...
	public TypeBuilder crs(String SRS) {
		try {
			...
			IllegalArgumentException iae = new IllegalArgumentException("SRS '"
					+ SRS + "' unknown:" + e);
			iae.initCause(e);
			throw iae;
		}
