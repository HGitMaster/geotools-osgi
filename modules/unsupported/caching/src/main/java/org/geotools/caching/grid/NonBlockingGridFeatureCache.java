...
/**
 * This is a implementation of a feature cache that does not block
 * while getting data to write to the cache.
 * 
 * <p>
 * The result is that multiple request may request the same data and the same data
 * may be placed in the cache multiple times.  However the performance is better
 * as other threads can still read the cache.
 * </p><p>
 * An improvement might be to try to track which regions have already been requested
 * and wait for those regions as opposed to just request them again.
 * </p>
 * 
...
public class NonBlockingGridFeatureCache extends GridFeatureCache {
     ...
     * @throws IOException 
     */
    public NonBlockingGridFeatureCache(FeatureSource fs, int indexcapacity, int capacity, Storage store)
        throws FeatureCacheException {
     ...
     */
    public NonBlockingGridFeatureCache(FeatureSource fs, ReferencedEnvelope env, int indexcapactiy, int capcity, Storage store){
    ...
    public FeatureCollection get(Envelope e) throws IOException { // TODO: read lock
        ...
        //this could all be done in a different thread?
        //so the thread that requested this data can continue.
