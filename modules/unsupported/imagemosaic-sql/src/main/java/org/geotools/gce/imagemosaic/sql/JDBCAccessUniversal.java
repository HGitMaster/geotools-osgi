...
class JDBCAccessUniversal extends JDBCAccessBase {
    ...
    private String matchSelectJoined;
    private String tileSelect;
    private String tileEnvelopeSelect;
    ...
    private void initStatementStrings(Config config) {
        ...
        matchSelect = String.format("SELECT s.%s FROM {0} s WHERE ",
                config.getKeyAttributeNameInSpatialTable());
        matchSelectJoined = String.format("SELECT s.%s FROM {0} s, {1} t WHERE  s.%s = t.%s AND ",
                config.getKeyAttributeNameInSpatialTable(),
                config.getKeyAttributeNameInSpatialTable(),
                config.getKeyAttributeNameInTileTable());
        
        matchSelect += whereClause;
        matchSelectJoined += whereClause;
        
        tileSelect = String.format("SELECT %s FROM {0} WHERE %s = ?",
                config.getBlobAttributeNameInTileTable(),
                config.getKeyAttributeNameInTileTable());
        
        tileEnvelopeSelect = String.format("SELECT 1, %s, %s, %s, %s FROM {0} WHERE %s = ?",
     ...
     */
    @Override
    protected String getExtentSelectStatement(ImageLevelInfo li) {
    ...
    @Override
    protected String getMatchSelectStatement(ImageLevelInfo li) {
        if (li.isImplementedAsTableSplit()) {
            return MessageFormat.format(matchSelectJoined,
                new Object[] { li.getSpatialTableName(), li.getTileTableName() });
        } else {
            return MessageFormat.format(matchSelect,
                new Object[] { li.getSpatialTableName() });
        }
    }
    
    @Override
    protected String getTileSelectStatement(ImageLevelInfo li)
    {
        return MessageFormat.format(tileSelect, li.getTileTableName());
    }
    
    @Override
    protected String getTileEnvelopeSelectStatement(ImageLevelInfo li)
    {
        return MessageFormat.format(tileEnvelopeSelect, li.getTileTableName());
    }
